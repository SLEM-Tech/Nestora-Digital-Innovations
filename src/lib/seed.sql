-- ============================================================
-- Nestora Seed Data — Premium Modern Furniture & Home Decor
...
FROM nestora_categories WHERE slug = 'seating-sofas' ON CONFLICT (slug) DO NOTHING;
...
SELECT id, 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=1000', 'Velvet Sofa', 'Emerald Green Velvet Nordic Sofa', 0 FROM nestora_products WHERE slug = 'nordic-velvet-sofa' UNION ALL
SELECT id, 'https://images.unsplash.com/photo-1567538096630-e0c55bd6374c?w=1000', 'Lounge Chair', 'Classic Leather Eames Style Lounge Chair', 0 FROM nestora_products WHERE slug = 'eames-lounge-chair' UNION ALL
SELECT id, 'https://images.unsplash.com/photo-1518005020453-6ec24af4b28c?w=1000', 'Arc Lamp', 'Solaris Large Arc Floor Lamp', 0 FROM nestora_products WHERE slug = 'solaris-arc-lamp' UNION ALL
SELECT id, 'https://images.unsplash.com/photo-1533090161767-e6ffed986c88?w=1000', 'Office Workspace', 'ErgoPro High Back Mesh Office Chair', 0 FROM nestora_products WHERE slug = 'ergopro-mesh-chair' UNION ALL
SELECT id, 'https://images.unsplash.com/photo-1538688525198-9b88f6f53126?w=1000', 'Sideboard', 'Mid-Century Walnut Sideboard Storage', 0 FROM nestora_products WHERE slug = 'walnut-sideboard'
ON CONFLICT DO NOTHING;

-- ── 4. Attributes (Material & Colors) ────────────────────────

INSERT INTO nestora_product_attributes (product_id, name, options, position)
SELECT id, 'Fabric Color', ARRAY['Emerald Green', 'Deep Navy', 'Slate Gray', 'Terracotta'], 0 FROM nestora_products WHERE slug = 'nordic-velvet-sofa' ON CONFLICT DO NOTHING;

INSERT INTO nestora_product_attributes (product_id, name, options, position)
SELECT id, 'Wood Finish', ARRAY['American Walnut', 'Natural Oak', 'Blackened Ash'], 0 FROM nestora_products WHERE slug = 'walnut-sideboard' ON CONFLICT DO NOTHING;

INSERT INTO nestora_product_attributes (product_id, name, options, position)
SELECT id, 'Size', ARRAY['Medium (6x8)', 'Large (8x10)', 'Extra Large (10x12)'], 0 FROM nestora_products WHERE slug = 'abstract-geometric-rug' ON CONFLICT DO NOTHING;
