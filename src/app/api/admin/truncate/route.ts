...
// Auth: admin cookie OR ?secret=nestora-truncate-2024
export async function DELETE(req: NextRequest) {
  const secret = req.nextUrl.searchParams.get("secret");
  const admin = await requireAdmin(req);
  if (!admin && secret !== "nestora-truncate-2024") {
  ...
  try {
    if (target === "products" || target === "all") {
      await query(`TRUNCATE ${T.productAttributes}, ${T.productImages}, ${T.productCategories}, ${T.orderItems}, ${T.products} RESTART IDENTITY CASCADE`);
    }
